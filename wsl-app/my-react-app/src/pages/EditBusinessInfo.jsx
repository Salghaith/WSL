...
const EditBusinessInfo = () => {
  ...
  const [businessAddress, setBusinessAddress] = useState({ city: 'New York', street: '5th Avenue' });
  const [operatingHours, setOperatingHours] = useState('9 AM - 5 PM');
  const [description, setDescription] = useState('Your one-stop shop for tech gadgets.');
  ...
  const validateInputs = () => {
    ...
    if (!emailRegex.test(businessEmail)) newErrors.businessEmail = 'Invalid business email format.';
    if (!phoneRegex.test(businessPhone)) newErrors.businessPhone = 'Phone number must be 10 digits.';
    if (!emailRegex.test(ownerEmail)) newErrors.ownerEmail = 'Invalid owner email format.';
